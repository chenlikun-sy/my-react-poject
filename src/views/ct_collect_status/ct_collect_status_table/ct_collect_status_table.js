...
export default class CtCollectStatusTable extends React.Component {
    ...
    setTableColumns() {
        ...
        theadJson.forEach((item) => {
            ...
            obj.key = item.id;
            obj.align = 'center';
            obj.sorter = (a, b) => a[item.id].length - b[item.id].length;
            obj.render = (text, record) => (
                <Tooltip placement="topLeft" title={text}>
                    {text}
                </Tooltip>
            );
            if (item.id === "dc_data_time" || item.id === "task_run_time") {
                obj.sorter = (a, b) => new Date(a[item.id]) - new Date(b[item.id])
            }
            if (item.id === "run_secs" || item.id === "dc_count" || item.id === "dc_rows" || item.id === "omc_file_rows") {
                obj.sorter = (a, b) => a[item.id] - b[item.id]
            }
